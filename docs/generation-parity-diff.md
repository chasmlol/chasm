# Generation parity diff — Rust vs SillyTavern (ground truth)
...
captured at the llama.cpp hop via `scripts/llm_capture_proxy.py`. SillyTavern is
the baseline (`scripts/captures/st/`), Rust is the candidate (`scripts/captures/`).
...
2. **Constant quest with no giver gate.** The one FNV quest (`constant:true`, given by
   ...
   givers only activates when the speaker is a giver (ST's `quest-books.js` rule).
   → quest over-activation (2.7k) gone.
3. **Structured quest/action instructions emitted on the *enable* flag**, not on actual
   activation. Fixed: gated on whether the block activated (ST behavior). → orphan
   1.7k action instruction gone.

**Remaining gap is vector retrieval only** (see §"What diverges" 3–4): lore now selects
a *different* (keyword-matched) set than ST's semantic set, and chat-vector recall is
...
1. **Action books over-activate in Rust (the dominant gap).**
   For "who are you", ST activated **zero** action entries (no action block at all);
   Rust emitted a **7,341-char `Activated Action Book entries:`** block. ST gates
   actions by *relevance* (vector/intent) — actions only appear when the player's
   line implies one (verified: ST scenario 2 "follow me" *did* include actions and
...
2. **Quest books over-activate in Rust.** ST: no quest block for this turn. Rust:
...
3. **Lore selects *different entries*.** Both emit ~9 entries of similar total size,
   but different ones. ST (vector/semantic): Ghost Town Gunfight, Run Goodsprings Run,
...
4. **Chat-vector memory missing in Rust.** ST has `Relevant past chat context:` (302,
   vector recall of earlier conversation); Rust has none (no embeddings path).

5. **`Response instructions:` (431) present in ST, absent in Rust.**

6. **Audio Tags block: ST includes it (1,136 chars); Rust omits it.**
...
7. **Structured-instruction wording + ordering differ.** ST appends one
...
8. **Model id**: Rust forwards the gguf file path as `model`; ST forwards the short
...
- **P0 — Gate action/quest activation by relevance.** Today Rust injects the full
  ...
  action-happy. Needs intent/keyword gating that matches ST's behavior (and ideally
  the vector path).
...
dynamic retrieval (action/quest/lore gating + chat vectors), which is where ST's
